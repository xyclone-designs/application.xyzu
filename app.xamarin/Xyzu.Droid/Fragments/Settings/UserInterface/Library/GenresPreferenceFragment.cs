...
namespace Xyzu.Fragments.Settings.UserInterface.Library
{
	...
	public class GenresPreferenceFragment : BasePreferenceFragment, IGenresSettings
	{
		public const string FragmentName = "Xyzu.Fragments.Settings.UserInterface.Library.GenresPreferenceFragment";

		public static class Keys										
		{
			public const string IsReversed = "settings_userinterface_library_genres_isreversed_switchpreference_key";
			public const string LayoutType = "settings_userinterface_library_genres_layouttype_listpreference_key";
			public const string SortKey = "settings_userinterface_library_genres_sortkey_listpreference_key";
		}
		...
		public bool IsReversed
		{
			...
			set
			{
				...
				XyzuSettings.Instance
					.Edit()?
					.PutBoolean(IGenresSettings.Keys.IsReversed, value)?
					.Apply();
		...
		public LibraryLayoutTypes LayoutType
		{
			...
			set
			{
				...
				XyzuSettings.Instance
					.Edit()?
					.PutEnum(IGenresSettings.Keys.LayoutType, value)?
					.Apply();
		...
		public ModelSortKeys SortKey
		{
			...
			set
			{
				...
				XyzuSettings.Instance
					.Edit()?
					.PutEnum(IGenresSettings.Keys.SortKey, value)?
					.Apply();
		...
		public override void OnResume()
		{
			...
			AppCompatActivity?.SetTitle(Resource.String.settings_userinterface_library_genres_title);
			...
			IGenresSettings settings = XyzuSettings.Instance.GetUserInterfaceLibraryGenres();
		...
		{
			SetPreferencesFromResource(Resource.Xml.settings_userinterface_library_genres, rootKey);
			InitPreferences(
		...
		public override void OnPropertyChanged([CallerMemberName] string? propertyName = null)
		{
			...
			switch (propertyName)
			{
				...
				LayoutTypePreference != null:
					if (IGenresSettings.Options.LayoutTypes.AsEnumerable().NeatlyOrdered().Index(LayoutType) is int layouttypeindex)
						LayoutTypePreference.SetValueIndex(layouttypeindex);
					break;

				case nameof(SortKey) when
				SortKeyPreference != null:
					if (IGenresSettings.Options.SortKeys.AsEnumerable().Index(SortKey) is int sortkeyindex)
