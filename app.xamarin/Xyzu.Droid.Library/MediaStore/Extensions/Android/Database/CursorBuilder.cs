...
using AndroidOSBuild = Android.OS.Build;
...
namespace Android.Database
{
	public class CursorBuilder
	{
		...
		private ILibraryDroid.IIdentifiers? Identifiers { get; set; }
		private string[]? Projection { get; set; }
		private string? Selection { get; set; }
		private string[]? SelectionArgs { get; set; }
		private ILibraryDroid.ISettings? Settings { get; set; }
		...
		public ICursor? Build()
		{
			...
			Uri ??= true switch
			{
				true when AndroidOSBuild.VERSION.SdkInt >= BuildVersionCodes.Q
		...
		public CursorBuilder WithLibraryIdentifiers(ILibraryDroid.IIdentifiers? identifiers, bool exclusive = true)
		{
		...
		public CursorBuilder WithSettings(ILibraryDroid.ISettings? settings)
		{
			Settings = settings;
